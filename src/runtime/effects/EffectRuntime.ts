...
interface IsolatedMaterial { mesh: Mesh; original: Material | Material[]; clones: Material[] }
let activeLoops = 0
...
export class EffectRuntime {
  ...
  setEffects(effects: readonly EffectInstance[]): void {
    ...
    for (const instance of this.instances) {
      ...
      if (instance.kind === 'outline') {
        ...
        if (typeof bid === 'string') { this.meteor.setOutline(bid, true); this.outlined.add(bid) }
      } else if (instance.kind === 'child-highlight') highlights.push({ target, instance })
      else this.visuals.push(this.createVisual(instance, target))
    }
  ...
  getDiagnostics(): { effects: number; helpers: number; isolatedMeshes: number; outlined: number; unresolved: string[] } {
    return { effects: this.instances.length, helpers: this.visuals.length, isolatedMeshes: this.materials.length, outlined: this.outlined.size, unresolved: [...this.unresolved] }
  }
  ...
  private applyHighlights(effects: Array<{ target: Object3D; instance: EffectInstance }>): void {
    ...
    for (const [mesh, effect] of byMesh) {
      ...
      this.materials.push({ mesh, original, clones })
    }
  }
  ...
  dispose(): void {
    if (this.disposed) return
    this.clear(); this.instances = []; this.root.removeFromParent(); this.disposed = true
  }
}
